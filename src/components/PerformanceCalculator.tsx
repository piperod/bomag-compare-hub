          ...
          {customWeight && selectedSoil && (
            <Card className="bg-blue-50">
              <CardContent className="pt-6">
                ...
                {(() => {
                  const recommendation = getRecommendationForCustomWeight(parseFloat(customWeight));
                  if (recommendation) {
                    return (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <h5 className="text-sm text-gray-600">Altura de capa (basado en rango {recommendation.weightRange}t)</h5>
                          <div className="text-xl font-bold text-bomag-orange">
                            {recommendation[selectedSoil as keyof CompactionData]} m
                          </div>
                        </div>
                        <div>
                          <h5 className="text-sm text-gray-600">Rendimiento (basado en rango {recommendation.weightRange}t)</h5>
                          <div className="text-xl font-bold text-bomag-blue">
                            {(() => {
                              const perfData = compactionPerformanceData.find(p => p.weightRange === recommendation.weightRange);
                              return perfData ? perfData[selectedSoil as keyof CompactionData] : '-';
                            })()} m³/h
                          </div>
                        </div>
                      </div>
